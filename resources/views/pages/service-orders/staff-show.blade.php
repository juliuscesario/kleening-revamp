...
@section('content')
    <style>
        .signature-pad-canvas {
            width: 100%;
            max-width: 500px;
            /* Limit max width for smaller saved images */
            height: 200px;
                        ...
                        @if($serviceOrder->status == 'booked')
                            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#startWorkModal">
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24"
                                    ...
                                    <path d="M12 7l-2 4l2 4l2 -4l-2 -4" />
                                </svg>
                                Mulai Kerja
                            </button>
                            ...
                            @if(!$hasBeforePhoto)
                                <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#uploadBeforeWorkProofModal">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24"
                                        ...
                                        <path d="M14 14l1 -1c.699 -.67 1.78 -.825 2.5 -.288" />
                                        <path d="M19 22v-6" />
                        ...
                        @elseif($serviceOrder->status == 'proses' && $serviceOrder->work_proof_completed_at && (!$serviceOrder->customer_signature_image || $serviceOrder->staff->whereNull('pivot.signature_image')->isNotEmpty()))
                            <button class="btn btn-success" id="requestSignatureBtn">
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24"
                                    ...
                                    stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                    <path d="M7 7c1.657 0 3 1.59 3 3s-1.657 3 -3 3s-3 -1.59 -3 -3s1.657 -3 3 -3" />
                                    <path d="M17 17c1.657 0 3 1.59 3 3s-1.657 3 -3 3s-3 -1.59 -3 -3s1.657 -3 3 -3" />
                                    <path d="M7 13v4a3 3 0 0 0 3 3h1" />
                                    <path d="M17 13v4a3 3 0 0 0 3 3h1" />
                                    <path d="M17 10h-1a2 2 0 0 0 -2 2v2a2 2 0 0 0 2 2h1" />
                                    <path d="M7 10h1a2 2 0 0 1 2 2v2a2 2 0 0 1 -2 2h-1" />
                                </svg>
                                Minta Tanda Tangan
                            </button>
                        @endif
        ...
        <div class="page-body">
            <div class="row g-4">
                        ...
                        <div class="card-body">
                            ...
                            <p><strong>Alamat:</strong>
                                @if ($serviceOrder->address)
                                    ...
                                    @if ($serviceOrder->address->google_maps_link)
                                        <a href="{{ $serviceOrder->address->google_maps_link }}"
                                            class="btn btn-sm btn-outline-primary ms-2" target="_blank">Buka Peta</a>
                                    @endif
                            ...
                            <p><strong>Nomor Handphone:</strong>
                                @if ($serviceOrder->customer && $serviceOrder->customer->phone_number)
                                    ...
                                    @endphp
                                    <a
                                        href="https://wa.me/{{ $serviceOrder->customer->phone_number }}">{{ $serviceOrder->customer->phone_number }}</a>
                                    @if ($normalizedPhone)
                                        <a href="https://wa.me/{{ $normalizedPhone }}" class="btn btn-sm btn-outline-success ms-2"
                            ...
                            <p><strong>Tanggal Pengerjaan:</strong>
                                {{ \Carbon\Carbon::parse($serviceOrder->work_date)->format('d M Y') }}</p>
                            <p><strong>Waktu Pengerjaan (WIB):</strong>
                                {{ $serviceOrder->work_time_formatted ? $serviceOrder->work_time_formatted . ' WIB' : 'Tidak diatur' }}
                            </p>
                            <p><strong>Status:</strong>
                                ...
                                @endphp
                                <span
                                    class="badge {{ $statusBadgeClass }} text-bg-secondary">{{ ucfirst($serviceOrder->status) }}</span>
                            </p>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card mt-4">
                        <div class="card-header">
                            <h3 class="card-title">Material Lapangan</h3>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table card-table table-vcenter">
                                    ...
                                    <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($expense->photo_path)
                                                        <span class="badge bg-success text-bg-success">Bon Sudah Ada</span>
                                                    @else
                                                        <button type="button" class="btn btn-sm btn-warning btn-upload-bon"
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="2" class="text-center text-muted">Belum ada material yang dicatat.
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-header">
                            <h5>Catatan Internal (untuk Staff)</h5>
                        </div>
                        <div class="card-body">
                ...
                <form id="uploadReceiptForm" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="file" class="d-none" id="receipt_photo" name="photo" accept="image/*" required>
                            <div class="d-grid gap-2">
                                <button type="button" class="btn btn-outline-primary" id="receiptTakePhotoBtn">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon me-2" width="24" height="24"
                                        viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                        stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                ...
                                <button type="button" class="btn btn-outline-primary" id="receiptSelectGalleryBtn">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon me-2" width="24" height="24"
                                        viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                        stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                            ...
                            <div class="mt-2" id="receiptPhotoPreview" style="display: none;">
                                <img src="" alt="Receipt Photo Preview" class="img-fluid rounded"
    ...
    <div class="modal modal-blur fade" id="signatureModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="signatureModalTitle">Tanda Tangan untuk Service Order
                ...
                <div class="modal-body">
                    <div id="customerSignatureSection">
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <button type="button" class="btn btn-primary" id="saveCustomerSignature">Simpan Tanda Tangan
                                Pelanggan</button>
                        </div>
                    </div>
                    ...
                    <div id="staffSignatureSection" style="display: none;">
                        ...
                        <div class="d-flex justify-content-end mt-3">
                            <button type="button" class="btn btn-primary" id="saveStaffSignature"
                                style="display: none;">Simpan Tanda Tangan Staff</button>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const receiptPhotoPreview = document.getElementById('receiptPhotoPreview');
            const receiptPhotoPreviewImg = receiptPhotoPreview ? receiptPhotoPreview.querySelector('img') : null;
            const receiptWorkProofLoading = document.getElementById('receiptWorkProofLoading');
            const receiptWorkProofSubmitBtn = uploadReceiptForm ? uploadReceiptForm.querySelector('button[type="submit"]') : null;

            // Handle buttons on table
            ...
            function toggleReceiptWorkProofLoading(isLoading) {
                if (receiptWorkProofLoading) receiptWorkProofLoading.classList.toggle('d-none', !isLoading);
                if (receiptWorkProofSubmitBtn) receiptWorkProofSubmitBtn.disabled = isLoading;
                if (receiptPhotoInput) receiptPhotoInput.disabled = isLoading;
            }

            if (receiptPhotoInput) {
                receiptPhotoInput.addEventListener('change', () => handleImageSelection(receiptPhotoInput, receiptPhotoPreview, receiptPhotoPreviewImg));
            }

            setupPhotoButtons('receiptTakePhotoBtn', 'receiptSelectGalleryBtn', 'receipt_photo');

            if (uploadReceiptModalEl) {
                uploadReceiptModalEl.addEventListener('hidden.bs.modal', () => {
                    toggleReceiptWorkProofLoading(false);
                    resetFileInput(receiptPhotoInput, receiptPhotoPreview, receiptPhotoPreviewImg);
                });
            }
            ...
            if (uploadReceiptForm) {
                uploadReceiptForm.addEventListener('submit', async (event) => {
                    ...
                    toggleReceiptWorkProofLoading(true);
                    try {
                        const processedFile = await ensureProcessedFile(receiptPhotoInput);
                        if (!processedFile) {
                            if (!receiptPhotoInput.files || receiptPhotoInput.files.length === 0) {
                                alert('Mohon unggah foto terlebih dahulu.');
                            }
                            toggleReceiptWorkProofLoading(false);
                            return;
                        ...
                        const data = await sendFormData(uploadUrl, formData);
                        alert(data.message || 'Foto Bon Material berhasil diunggah.');
                        location.reload();
                    } catch (error) {
                        console.error('Error:', error);
                        alert(error.message || 'Gagal mengunggah foto Bon Material.');
                        toggleReceiptWorkProofLoading(false);
                    }
                });
            }
